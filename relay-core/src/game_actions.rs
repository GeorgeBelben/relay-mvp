...
/// Backs a game's "Achievements" view and any Home progress card. `badge_url` is already resolved
/// to the locked/unlocked variant server-side so a caller never needs to know RA's own
/// badge-naming convention. Mirrors the Electron MVP's `RetroAchievementsProgress` shared type.
#[derive(Debug, PartialEq, Serialize)]
...
pub enum GameActionsError {
    ...
    #[error("RetroAchievements error: {0}")]
    RetroAchievements(#[from] RaError),
...
// Not a #[from] -- EnrichError's variants get unpacked into the matching GameActionsError variant
// rather than wrapped in a new one, since GameActionsError already has its own Download/Io
...
/// Free-text search against SteamGridDB -- backs a "Reidentify" flow, for when the automatic
/// match picked the wrong game (or none at all). The caller owns what query to send (typically
/// prefilled from the game's filename-derived `scanned_title`, but freely editable -- searching by
/// a bad match's own current title would be self-defeating, since that title IS the bad match's
/// name), so this takes the query directly rather than resolving one from a game_id.
...
pub async fn apply_reidentify(
    ...
    let dest_dir = library::game_media_dir(media_root, &system_id, game_id);
