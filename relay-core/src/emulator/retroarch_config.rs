//! Generates the per-launch `--appendconfig` file for RetroArch, overlaying per-game
...
// Relay owns the whole "leave this game" experience end to end: pause, save, and quit all go
// through the quick menu's own UI, driven over the network command port above, not RetroArch's
// own input handling or menu. Forced here on every launch for the same reason as
...
// The on-load "core name + content title" banner -- confirmed against real user reports of this
// exact combination, since none of these are individually documented as "the" load banner toggle:
...
/// The subset of Settings that affect how a game actually plays/looks, as opposed to
...
mod tests {
    ...
    fn append_config_contents_disables_retroarchs_own_exit_menu_load_banner_and_config_save() {
        ...
        // The quick menu is meant to be the only way out of a running game -- these keep
        // RetroArch's own exit hotkey and menu from ever being reachable, and the icon+title
        // banner from ever popping up on load, regardless of what a base retroarch.cfg (or a
        // future RetroArch default change) might otherwise set. config_save_on_exit keeps
