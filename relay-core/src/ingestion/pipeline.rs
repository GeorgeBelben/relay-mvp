...
async fn upsert_rom_and_game(
    ...
    let dir = library::game_media_dir(media_root, system_id, &game.id);
...
/// finds, and upserts roms/games. Anything not found this pass is marked missing, not deleted.
...
/// than a second background process a caller can't reflect. `running` guards against overlapping
/// runs (a second "Rescan" call mid-run is a no-op, not an error) -- callers own the flag's
...
/// production wires it to a UI event; tests just collect it.
