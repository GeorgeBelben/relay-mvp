...
//! talk to) -- done here as an app-local encrypted file instead: a random key generated once on
//! first use, stored with owner-only permissions, deliberately separate from the SQLite DB itself
//! so a DB copy/backup alone doesn't leak credentials. This defends against casual disk/backup
//! exposure, which is the actual threat model for a personal single-user kiosk -- not a
//! sophisticated adversary who'd already have full device access (and therefore the app's own
//! decrypted view) anyway.
