//! The fixed, complete set of emulated systems Relay supports -- ported verbatim from
//! relay-ui's `systems.rs` (2026-09-08). This is source code, not seeded database rows: the
//! set of systems a console supports is a product decision, not user-editable data, so there's
//! no `create`/`update`/`delete` for it -- only `get`/`all` lookups. `roms.system_id` stores one
//! of these `id`s as a plain string with no DB-level foreign key.
//!
//! This is a starting list, not full RetroArch/libretro coverage -- see REL ticket for expanding
//! it properly (verifying real core names and Ubuntu package availability per system) rather than
//! guessing at a "complete" list inline.
//!
//! Extensions are matched case-insensitively without a leading dot. CD-based systems (psx,
//! saturn, dreamcast) deliberately omit "bin" -- it's a data track referenced BY the .cue/.gdi
//! wrapper, not launchable on its own, so listing it would double-index every single-disc CD
//! game.
//!
//! `retroarch_core`/`standalone_binary`: exactly one is set per system. These are sensible
//! defaults, not verified against real hardware for every entry -- swap a system's core here if a
//! different one turns out to fit better once this runs on the actual device.
...
pub const ALL: &[SystemDef] = &[
    ...
    SystemDef {
        ...
        // relay-ui REL-136) -- desmume is, and is the standard fallback libretro NDS core.
    ...
    SystemDef {
        ...
        // device, relay-ui REL-136) -- mednafen_psx ("Beetle PSX", package libretro-beetle-psx)
        // is.
    ...
    SystemDef {
        ...
        // against the real device, relay-ui REL-136) -- yabause-qt is the closest packaged
        // standalone.
        retroarch_core: None,
