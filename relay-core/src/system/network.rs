// WiFi settings, ported from the Electron MVP's network.service.ts / lib/network/nmcli.ts.
// Parsing/classification logic is pure and fully unit-tested; the actual `nmcli` execution
// (list_wifi_networks/connect_to_wifi_network) is thin glue over it and, like bluetoothctl, needs
// real Linux hardware with NetworkManager to verify end to end -- there's no WiFi adapter or nmcli
