// Mock implementation used on every non-Linux target so this crate (and anything built on it)
// runs standalone on macOS. Reads fixture data from a TOML file (RELAY_MOCK_FIXTURE env var, or
// dev-fixtures/default.toml alongside this crate if unset) instead of hardcoding it, so switching
// between states (empty/degraded/happy-path) doesn't need a recompile -- see dev-fixtures/ in this
// crate's root.
//
...
// A representative slice of IANA zones, enough to exercise a timezone picker's search/filter UI
...
mod tests {
    ...
    // field before it turns into a first-use panic during local dev.
    #[test]
