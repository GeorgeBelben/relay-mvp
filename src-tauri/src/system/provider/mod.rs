...
// running Ubuntu Server + cage, with NetworkManager/BlueZ/systemd installed) so `cargo tauri dev`
// can also run directly on macOS for UI iteration -- see dev-fixtures/ and CLAUDE.md for the
// RELAY_MOCK_FIXTURE workflow. Selection happens once at compile time via #[cfg(target_os)], not
// as a runtime branch: every call site just goes through `ActiveSystemProvider`, which is a type
// alias picking `linux::LinuxSystemProvider` or `mock::MockSystemProvider`.
//
// Storage (system::storage, via nix::statvfs) isn't behind this trait -- it already works
// unmodified on macOS, so mocking it would be pure overhead. Gamepad/controller input isn't here
// either: it's read via the browser Gamepad API on the frontend (src/lib/input/), which is
...
// future is guaranteed Send -- required since Tauri's command dispatch awaits these from a
// multi-threaded runtime, and plain `async fn` in a trait can't express that bound.
