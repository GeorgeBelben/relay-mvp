...
use crate::ingestion::paths::{self, IMAGE_EXTENSIONS};
...
impl std::fmt::Display for GameMediaFilesError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::GameNotFound => write!(f, "game not found"),
            Self::InvalidFilename => write!(f, "invalid filename"),
            Self::FileNotFound => write!(f, "file not found in this game's media folder"),
            Self::Db(e) => write!(f, "database error: {e}"),
        }
    }
}

impl std::error::Error for GameMediaFilesError {}

impl From<sqlx::Error> for GameMediaFilesError {
...
pub async fn list_media_files(pool: &SqlitePool, media_root: &Path, game_id: &str) -> Result<Vec<String>, GameMediaFilesError> {
    ...
    let dir = paths::game_media_dir(media_root, &system_id, game_id);
...
pub async fn select_boxart_file(pool: &SqlitePool, media_root: &Path, game_id: &str, filename: &str) -> Result<(), GameMediaFilesError> {
    ...
    let local_path = paths::to_forward_slash(file_path.strip_prefix(media_root).unwrap_or(&file_path));
